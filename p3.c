...
#define GET_4B(a,o) ((INT32)UCH(a[o]) + (((INT32)UCH(a[o+1]))<<8) \
                    + (((INT32)UCH(a[o+2]))<<16) + (((INT32)UCH(a[o+3]))<<24))
...
int main(){
    ...
    // 2. 中心化並複製到 COMPLEX 陣列
    COMPLEX *c = malloc(sizeof(COMPLEX) * width * height);
    for(int y=0; y<height; y++){
        int row = y * stride;
        for(int x=0; x<width; x++){
            int idx = y*width + x;
            double val = data[row + x];
            // center shift: multiply by (-1)^(x+y)
            if ((x + y) & 1) val = -val;
            c[idx].real = val;
            c[idx].imag = 0.0;
        }
...
void ReadImageData(const char *name,
    ...
    if(colTbl) fread(colTbl, 1, ctSize, fp);
